...
from broker.jainamprop.baseurl import MARKET_DATA_URL
...
def download_csv_jainamprop_data(output_path):
...
def process_jainamprop_nse_csv(path):
    """
...
def process_jainamprop_bse_csv(path):
    """
    Processes the JainamXTS CSV file to fit the existing database schema and performs exchange name mapping.
    """
...
def process_jainamprop_nfo_csv(path):
    """
    Processes the JainamXTS CSV file to fit the existing database schema and performs exchange name mapping.
    """
...
def process_jainamprop_cds_csv(path):
    """
    Processes the JainamXTS CSV file to fit the existing database schema and performs exchange name mapping.
    """
...
def process_jainamprop_bfo_csv(path):
    """
    Processes the JainamXTS CSV file to fit the existing database schema and performs exchange name mapping.
    """
...
def process_jainamprop_mcx_csv(path):
    """
    Processes the JainamXTS CSV file to fit the existing database schema and performs exchange name mapping.
    """
...
def delete_jainamprop_temp_data(output_path):
...
def master_contract_download():
    ...
    output_path = "tmp"
    try:
        download_csv_jainamprop_data(output_path)
        delete_symtoken_table()

        # Process each segment with individual error handling
        try:
            token_df = process_jainamprop_nse_csv(output_path)
            copy_from_dataframe(token_df)
        except Exception as e:
            logger.error(f"Error processing NSE CSV: {e}")

        try:
            token_df = process_jainamprop_bse_csv(output_path)
            copy_from_dataframe(token_df)
        except Exception as e:
            logger.error(f"Error processing BSE CSV: {e}")

        try:
            token_df = process_jainamprop_nfo_csv(output_path)
            copy_from_dataframe(token_df)
        except Exception as e:
            logger.error(f"Error processing NFO CSV: {e}")

        try:
            token_df = process_jainamprop_bfo_csv(output_path)
            copy_from_dataframe(token_df)
        except Exception as e:
        ...
        delete_jainamprop_temp_data(output_path)
