...
def transform_positions_data(positions_data):
    # Accept both dict and list responses; gracefully handle error/null payloads
    # Expected formats:
    # - {"positionList":[...]}
    # - [...]
    # - {"type":"error", ...} or {"result": null}
    if positions_data is None:
        return []
    if isinstance(positions_data, dict):
        # If broker returns error envelope or empty result, return empty list
        if positions_data.get("type") in ("error", False) or positions_data.get("result") is None:
            logger.info("No positions available (error or null result).")
            return []
        # Some handlers pass only {"positionList": [...]}
        positions_data = positions_data.get("positionList", positions_data.get("result", []))
    # If it is already a list, use as-is
    if isinstance(positions_data, list):
        positions_list = positions_data
    else:
        # Unexpected shape; log and return empty
        logger.error(f"Unexpected positions_data shape: {type(positions_data)}")
        positions_list = []
    transformed_data = []
    # Define exchange mappings
    exchange_mapping = {
    ...
    # positions_list is guaranteed to be a list now
