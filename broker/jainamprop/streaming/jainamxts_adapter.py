...
from broker.jainamprop.streaming.jainamxts_websocket import JainamXTSWebSocketClient
...
class JainamPropWebSocketAdapter(BaseBrokerWebSocketAdapter):
    """Jainam Prop (XTS) WebSocket adapter"""

    def __init__(self):
        super().__init__()
        self.logger = logging.getLogger("jainamprop_websocket")
        self.ws_client = None
        self.user_id = None
        self.broker_name = "jainamprop"
        ...
        self.logger.info(f"Jainam Prop adapter initialized with ZMQ port: {self.zmq_port}")

    def initialize(
        self, broker_name: str, user_id: str, auth_data: dict[str, str] | None = None
    ) -> None:
        """
        Initialize connection with Jainam Prop XTS WebSocket API

        Args:
            broker_name: Broker id (typically 'jainamprop')
        ...
        if not auth_data:
            ...
            if not api_key or not api_secret:
                ...
                raise ValueError("Missing Jainam Prop (XTS) API credentials in environment variables")
        ...
        self.logger.debug(f"Using API Key: {api_key[:10]}... for Jainam Prop XTS connection")
