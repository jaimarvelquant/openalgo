...
from broker.jainamprop.api.auth_api import get_feed_token as refresh_feed_token
from broker.jainamprop.baseurl import MARKET_DATA_URL
from broker.jainamprop.database.master_contract_db import SymToken, db_session
...
class BrokerData:
    def __init__(self, auth_token, feed_token=None, user_id=None):
        """Initialize jainamprop data handler with authentication token"""
        ...
        # Map common timeframe format to jainamprop intervals
        self.timeframe_map = {
            "1s": "1",
            "1m": "60",
            "2m": "120",
    ...
    def _fetch_market_data(
        ...
        """
        Helper method to fetch market data from jainamprop API
        Args:
    ...
    def _process_multiquotes_batch(self, symbols: list) -> list:
        ...
        try:
            ...
            if not response or response.get("type") != "success":
                ...
                raise Exception(f"Error from jainamprop API: {error_msg}")
    ...
    def get_history(self, symbol, exchange, timeframe, from_date, to_date):
        ...
        try:
            ...
            while current_start <= to_date:
                ...
                # jainamprop expects MMM DD YYYY HHMMSS in IST
                    ...
                    if "Invalid Token" in error_msg:
                        ...
                        if self._refresh_feed_token():
                            ...
                            if not response or response.get("type") != "success":
                                ...
                                raise Exception(
                                    f"Error from jainamprop API: {response.get('description', 'Unknown error') if response else 'No response'}"
                                )
                        else:
                            logger.error("Failed to refresh feed token")
                            raise Exception(f"Error from jainamprop API: {error_msg}")
