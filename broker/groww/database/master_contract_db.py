...
def format_marvelquant_to_groww_symbol(symbol, exchange):
    """
    Convert MarvelQuant symbol format to Groww symbol format
    ...
    """
    logger.info(f"Converting symbol from MarvelQuant to Groww format: {{symbol}}, {exchange}")
...
def format_groww_to_marvelquant_symbol(groww_symbol, exchange):
    """
    Convert Groww symbol format to MarvelQuant symbol format
    ...
    """
    logger.info(f"Converting symbol from Groww to MarvelQuant format: {{groww_symbol}}, {exchange}")
    ...
    if exchange == 'NFO':
        ...
        # If already in MarvelQuant format (no spaces), return as is
        ...
        if len(parts) >= 4 and parts[-1] in ('CE', 'PE'):
            ...
            # Combine into MarvelQuant format: [BaseSymbol][ExpirationDate][StrikePrice][OptionType]
            # Example: AARTIIND29MAY25630CE
            marvelquant_symbol = f"{base_symbol}{date_str}{strike_price}{option_type}"
            logger.info(f"Converted to MarvelQuant format: {marvelquant_symbol}")
        ...
        # For futures
            ...
            # Combine into MarvelQuant format: [BaseSymbol][ExpirationDate]FUT
            # Example: NIFTY29MAY25FUT
            marvelquant_symbol = f"{base_symbol}{date_str}FUT"
            logger.info(f"Converted to MarvelQuant format: {marvelquant_symbol}")
            return marvelquant_symbol
        ...
        # Handle case where option type might be missing
            ...
            if is_date:
                try:
                    ...
                    marvelquant_symbol = f"{base_symbol}{date_str}{strike_price}{option_type}"
                    logger.info(f"Converted to MarvelQuant format (assumed {{option_type}}): {marvelquant_symbol}")
                    return marvelquant_symbol
                except ValueError:
...
def find_token_by_symbol(symbol, exchange):
    ...
    if exchange == 'NFO':
        # Try with MarvelQuant format if it was in Groww format
        marvelquant_symbol = format_groww_to_marvelquant_symbol(symbol, exchange)
        if marvelquant_symbol != symbol:
            result = db_session.query(SymToken).filter_by(symbol=marvelquant_symbol, exchange=exchange).first()
            if result:
                return result.token
        
        # Try with Groww format if it was in MarvelQuant format
...
def process_groww_data(path):
    ...
    try:
        ...
        # Format the symbol for F&O (NFO) instruments to match MarvelQuant format
        def format_fo_symbol(row):
            ...
            # For segment='FNO', format according to MarvelQuant standard
    ...
    # Map instrument types to MarvelQuant standard types
    instrument_type_map = {
        'EQUITY': 'EQ',
        'INDEX': 'INDEX',
        'FUTURE': 'FUT',
    ...
    # Map exchanges to MarvelQuant standard exchanges
    exchange_map = {
        'NSE': 'NSE',
        'BSE': 'BSE',
        'NFO': 'NFO',
    ...
    # For F&O instruments, format the symbol in MarvelQuant format
...
def master_contract_download():
    ...
    try:
        ...
        # Step 5: Add MarvelQuant symbols where needed (converting Groww format to MarvelQuant format)
        ...
        for idx, row in nfo_options.iterrows():
            # Convert the broker symbol to MarvelQuant format if spaces are detected
            if ' ' in row['brsymbol']:
                token_df.at[idx, 'symbol'] = format_groww_to_marvelquant_symbol(row['brsymbol'], 'NFO')
