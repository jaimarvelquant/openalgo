...
from broker.groww.database.master_contract_db import format_marvelquant_to_groww_symbol, format_groww_to_marvelquant_symbol
...
def direct_get_order_book(auth):
    ...
    try:
        ...
        # Convert all symbols from Groww format to MarvelQuant format
        for order in all_orders:
            if 'trading_symbol' in order:
                ...
                # First, determine the correct MarvelQuant exchange
                ...
                if any(suffix in groww_symbol for suffix in ['CE', 'PE', 'C', 'P']):
                    exchange = 'NFO'
                    is_derivative = True
                    order['exchange'] = 'NFO'  # Set MarvelQuant exchange format
                ...
                if is_derivative:
                    ...
                    try:
                        ...
                        if token:
                            marvelquant_symbol = get_oa_symbol(token, 'NFO')
                            logger.info(f"MarvelQuant Symbol: {marvelquant_symbol}")
                            if marvelquant_symbol:
                                order['symbol'] = marvelquant_symbol
                                logger.info(f"Converted NFO symbol by token: {groww_symbol} -> {marvelquant_symbol}")
                        ...
                        try:
                            ...
                            if not is_future:
                                ...
                                if match:
                                    ...
                                    # Format as MarvelQuant expects: NIFTY15MAY2526650CE
                                    marvelquant_symbol = f"{symbol_name}{day}{month_name}{year}{strike}{option_type}"
                                    order['symbol'] = marvelquant_symbol
                                    logger.info(f"Converted Groww option symbol by pattern: {groww_symbol} -> {marvelquant_symbol}")
                                    symbol_converted = True
                            ...
                            # For Futures: Convert from "NIFTY2551FUT" to "NIFTY29MAY25FUT"
                                ...
                                if match:
                                    ...
                                    # Format as MarvelQuant expects: NIFTY29MAY25FUT
                                    marvelquant_symbol = f"{symbol_name}{day}{month_name}{year}FUT"
                                    order['symbol'] = marvelquant_symbol
                                    logger.info(f"Converted Groww futures symbol by pattern: {groww_symbol} -> {marvelquant_symbol}")
                                    symbol_converted = True
                        except Exception as e:
...
def get_trade_book(auth):
    ...
    try:
        ...
        # Format trades to match MarvelQuant's expected format (as used in the REST API)
        ...
        for trade in all_trades:
            ...
            marvelquant_trade = {
                # Fields expected by MarvelQuant's UI
        ...
        if marvelquant_trades:
            logger.info(f"Sample MarvelQuant trade format: {json.dumps(marvelquant_trades[0], indent=2, default=str)}")
        ...
        response = {
            ...
            'data': marvelquant_trades,  # This is what map_trade_data will look for first
            'tradebook': marvelquant_trades,  # For compatibility with different naming conventions
        ...
        try:
            ...
            if response_obj.status_code == 200 and response_data.get('status') == 'SUCCESS':
                ...
                if 'payload' in response_data and 'positions' in response_data['payload']:
                    ...
                    # Transform positions to match MarvelQuant's expected format
                    for position in raw_positions:
                        ...
                        marvelquant_symbol = groww_symbol
                        symbol_converted = False
                        ...
                        try:
                            ...
                            try:
                                ...
                                from marvelquant.database.token_db import get_oa_symbol
                            
                            # First try database lookup for any symbol
                            db_symbol = get_oa_symbol(groww_symbol, 'NFO')
                            if db_symbol:
                                marvelquant_symbol = db_symbol
                                logger.info(f"Database: Converted Groww symbol: {groww_symbol} -> {marvelquant_symbol}")
                                symbol_converted = True
                            else:
                                ...
                                if option_match:
                                    ...
                                    # Format as MarvelQuant expects: NIFTY15MAY2526650CE
                                    marvelquant_symbol = f"{symbol_name}{day}{month_name}{year}{strike}{option_type}"
                                    logger.info(f"Pattern: Converted Groww option symbol: {groww_symbol} -> {marvelquant_symbol}")
                                    symbol_converted = True
                                else:
                                    ...
                                    if future_match:
                                        ...
                                        # Format as MarvelQuant expects: NIFTY29MAY25FUT
                                        marvelquant_symbol = f"{symbol_name}{day}{month_name}{year}FUT"
                                        logger.info(f"Pattern: Converted Groww futures symbol: {groww_symbol} -> {marvelquant_symbol}")
                                        symbol_converted = True
                        ...
                        # Map exchange to MarvelQuant format
                        exchange = position.get('exchange', '')
                        if exchange == 'NSE':
                            marvelquant_exchange = 'NSE_EQ'
                        elif exchange == 'BSE':
                            marvelquant_exchange = 'BSE_EQ'
                        elif exchange == 'NFO':
                            marvelquant_exchange = 'NSE_FO'
                        else:
                            marvelquant_exchange = exchange
                            
                        # Create position object in MarvelQuant format
                        ...
                        if position.get('segment') == 'CASH':
                            ...
                            position_symbol = marvelquant_symbol  # Use converted symbol for other segments
                            
                        transformed_position = {
                            # Standard MarvelQuant fields
                            'symbol': position_symbol,
                            'tradingsymbol': position_symbol,
                            'exchange': marvelquant_exchange,
                            ...
                            'segment': 'EQ',  # MarvelQuant format for CASH segment
                            
                            # Specific Groww fields (renamed to match MarvelQuant expectations)
                        ...
                        for position in fno_positions:
                            ...
                            if position.get('segment') == 'FNO' or position.get('exchange') == 'NFO':
                                try:
                                    ...
                                    if db_symbol:
                                        marvelquant_symbol = db_symbol
                                        logger.info(f"Database: Converted Groww FNO symbol: {groww_symbol} -> {marvelquant_symbol}")
                                        symbol_converted = True
                                    else:
                                        ...
                                        # MarvelQuant format: "NIFTY13MAY2534000CE" or "BANKNIFTY13MAY2532500PE"
                                    ...
                                    if match:
                                        ...
                                        # Format as MarvelQuant expects: NIFTY15MAY2526650CE
                                        marvelquant_symbol = f"{symbol_name}{day}{month_name}{year}{strike}{option_type}"
                                        logger.info(f"Pattern: Converted Groww option position symbol: {groww_symbol} -> {marvelquant_symbol}")
                                        symbol_converted = True
                                    ...
                                    # For Futures: Convert from "NIFTY2551FUT" to "NIFTY29MAY25FUT"
                                        ...
                                        if match:
                                            ...
                                            # Format as MarvelQuant expects: NIFTY29MAY25FUT
                                            marvelquant_symbol = f"{symbol_name}{day}{month_name}{year}FUT"
                                            logger.info(f"Pattern: Converted Groww futures position symbol: {groww_symbol} -> {marvelquant_symbol}")
                                            symbol_converted = True
                                except Exception as e:
                                    logger.error(f"Error converting position symbol: {e}")
                                    # Fall back to original symbol if conversion fails
                            ...
                            exchange = position.get('exchange', '')
                            if exchange == 'NSE':
                                marvelquant_exchange = 'NSE'
                            elif exchange == 'BSE':
                                marvelquant_exchange = 'BSE'
                            elif exchange == 'NFO':
                                marvelquant_exchange = 'NSE_FO'
                            else:
                                marvelquant_exchange = exchange
                            ...
                            transformed_position = {
                                ...
                                'tradingsymbol': marvelquant_symbol,
                                'exchange': marvelquant_exchange,
                                'product': position.get('product', ''),
                                'quantity': net_qty,
                                'net_quantity': net_qty,
                                ...
                                'buy_quantity': buy_qty,
                                'sell_quantity': sell_qty,
                                'segment': 'FO',  # MarvelQuant format for FNO segment
                ...
                if 'holdings' in response_data['payload']:
                    ...
                    for holding in raw_holdings:
                        transformed_holding = {
                            ...
                            # Using the key names MarvelQuant expects
...
def direct_place_order_api(data, auth):
    ...
    try:
        ...
        if db_record and db_record.brsymbol:
            ...
            trading_symbol = format_marvelquant_to_groww_symbol(original_symbol, original_exchange)
        ...
        if resp.status_code == 200:
            ...
            if response_data.get("status") == "SUCCESS":
                ...
                formatted_response = {
                    ...
                    "trading_symbol": trading_symbol,
                    "symbol": original_symbol  # Add original MarvelQuant symbol to response
                }
...
def get_holdings(auth):
    ...
    try:
        ...
        # Transform holdings to MarvelQuant format
...
def cancel_order(orderid, auth, segment=None, symbol=None, exchange=None):
    ...
    try:
        ...
        if response_obj.status_code == 200:
            ...
            # If symbol is provided, include it in MarvelQuant format in the response
            if symbol:
                # Add the original MarvelQuant format symbol to the response
                response['symbol'] = symbol
                logger.info(f"Including MarvelQuant symbol in cancel response: {symbol}")
...
def direct_modify_order(data, auth):
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            # If symbol was provided in the original request, include it in MarvelQuant format
            if 'symbol' in data and data['symbol']:
                response['symbol'] = data['symbol']
                logger.info(f"Including MarvelQuant symbol in modify response: {data['symbol']}")
            ...
            if order_status.upper() in [s.upper() for s in cancellable_statuses]:
                try:
                    ...
                    if isinstance(cancel_response, dict) and cancel_response.get('status') == 'success':
                        ...
                        # Get and include symbol in the MarvelQuant format
                        if 'symbol' in order:
                            broker_symbol = order.get('symbol', '')
                            
                            # For NFO symbols that have spaces, convert to MarvelQuant format
                            exchange = order.get('exchange', 'NSE')
                            if exchange == 'NFO' and ' ' in broker_symbol:
                                try:
                                    from broker.groww.database.master_contract_db import format_groww_to_marvelquant_symbol
                                    marvelquant_symbol = format_groww_to_marvelquant_symbol(broker_symbol, exchange)
                                    if marvelquant_symbol:
                                        cancelled_item['symbol'] = marvelquant_symbol
                                        cancelled_item['brsymbol'] = broker_symbol  # Keep original broker symbol for reference
                                        logger.info(f"Transformed cancelled order symbol for UI: {broker_symbol} -> {marvelquant_symbol}")
                                except Exception as e:
