...
class GrowwExchangeMapper:
    """Maps MarvelQuant exchange codes to Groww exchange/segment format"""
    
    # Mapping from MarvelQuant exchange to Groww exchange and segment
    EXCHANGE_MAP = {
    ...
    def get_exchange(cls, marvelquant_exchange: str) -> str:
        """
        Get Groww exchange from MarvelQuant exchange code
        
        Args:
            marvelquant_exchange: MarvelQuant exchange code (e.g., 'NSE', 'NFO')
            
        Returns:
            str: Groww exchange code
        """
        mapping = cls.EXCHANGE_MAP.get(marvelquant_exchange, {})
        return mapping.get('exchange', marvelquant_exchange)
    
    @classmethod
    def get_segment(cls, marvelquant_exchange: str) -> str:
        """
        Get Groww segment from MarvelQuant exchange code
    ...
    def get_exchange_segment(cls, marvelquant_exchange: str) -> tuple:
        """
        Get both exchange and segment from MarvelQuant exchange code
        
        Args:
            marvelquant_exchange: MarvelQuant exchange code
            
        Returns:
            tuple: (exchange, segment)
        """
        mapping = cls.EXCHANGE_MAP.get(marvelquant_exchange, {})
        return mapping.get('exchange', marvelquant_exchange), mapping.get('segment', 'CASH')
