...
class Multitrade:
    ...
    def place_order(self, exch, token, tt, ot, qty, price):
        ...
        if res["status"] in ("sucess", "success", "successful"):
    ...
    def get_master_contract(self):
        ...
        df = pd.read_csv(io.StringIO(res.text), names=["sec_id", "sec_id_2", "symbol", "sec_description", "prev_close", "expiry_date", "strike_price", "tick_size", "quantity", "option_type", "instrument_type", "exchange"], header=None, index_col=False)
