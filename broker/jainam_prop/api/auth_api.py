...
def authenticate_broker(request_token):
    ...
    # Validate required credentials - raise ValueError to fail fast
    if not interactive_api_key:
        raise ValueError("JAINAM_INTERACTIVE_API_KEY environment variable is not set. Please configure Jainam Interactive API credentials.")
    if not interactive_api_secret:
        raise ValueError("JAINAM_INTERACTIVE_API_SECRET environment variable is not set. Please configure Jainam Interactive API credentials.")
...
def authenticate_market_data():
    ...
    if not market_api_key:
        raise ValueError("JAINAM_MARKET_API_KEY environment variable is not set. Please configure Jainam Market Data API credentials.")
    if not market_api_secret:
        raise ValueError("JAINAM_MARKET_API_SECRET environment variable is not set. Please configure Jainam Market Data API credentials.")
