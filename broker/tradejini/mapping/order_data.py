...
def transform_order_data(orders):
    """
    Processes and modifies a list of order dictionaries into the final MarvelQuant format.
...
def map_trade_data(trade_data):
    ...
    # Handle MarvelQuant format with status and data fields
    ...
    if trades_data:
        for trade in trades_data:
            ...
            mapped_trade = {
                ...
                "sym_id": symbol.get('id', '') # Store symbol ID for MarvelQuant lookup
            }
...
def transform_tradebook_data(trades):
    """
    Transforms mapped trade data to MarvelQuant format.
    ...
    # Check if already in MarvelQuant format
    if isinstance(trades, dict) and 'status' in trades and 'data' in trades:
        logger.debug("transform_tradebook_data - Already in MarvelQuant format")
    ...
    for trade in trades:
        ...
        # Get MarvelQuant symbol if possible
        try:
...
def map_position_data(position_data):
    ...
    for position in position_data:
        try:
            ...
            # Get MarvelQuant symbol - follow same approach as the main implementation
            marvelquant_symbol = None
            try:
...
def transform_holdings_data(holdings_data):
    """
    Transforms Tradejini holdings data to MarvelQuant format.
