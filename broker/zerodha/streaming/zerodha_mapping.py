...
class ZerodhaExchangeMapper:
    """Maps exchange codes between Zerodha and MarvelQuant formats"""
    
    # Map MarvelQuant exchange codes to Zerodha exchange codes
    _OA_TO_ZERODHA = {
        'NSE': 'NSE',
        'NFO': 'NFO',
        'CDS': 'CDS',
    ...
    # Map Zerodha exchange codes to MarvelQuant exchange codes
    _ZERODHA_TO_OA = {v: k for k, v in _OA_TO_ZERODHA.items()}
    
    @classmethod
    def to_zerodha_exchange(cls, oa_exchange: str) -> str:
        """
        Convert MarvelQuant exchange code to Zerodha exchange code.
        
        Args:
            oa_exchange: MarvelQuant exchange code (e.g., 'NSE', 'NFO')
    ...
    def to_oa_exchange(cls, zerodha_exchange: str) -> str:
        """
        Convert Zerodha exchange code to MarvelQuant exchange code.
...
class ZerodhaCapabilityRegistry:
    ...
    # Map MarvelQuant capability flags to Zerodha subscription modes
    CAPABILITY_MAP = {
        'LTP': 'ltp',
        'QUOTE': 'quote',
        'DEPTH': 'full',
    ...
    def get_zerodha_mode(cls, capability: str) -> str:
        ...
        Args:
            capability: MarvelQuant capability (LTP, QUOTE, DEPTH)
...
class ZerodhaDataTransformer:
    """Transforms data between Zerodha and MarvelQuant formats"""
    ...
    def transform_tick(self, tick_data: Dict, symbol: str, exchange: str) -> Dict:
        """
        Transform Zerodha tick data to MarvelQuant format.
    ...
    def transform_order_update(self, order_data: Dict) -> Dict:
        """
        Transform Zerodha order update to MarvelQuant format.
        ...
        try:
            ...
            # Map Zerodha status to MarvelQuant status
            status_map = {
                'OPEN': 'open',
                'COMPLETE': 'complete',
                'CANCELLED': 'cancelled',
    ...
    def transform_position(self, position_data: Dict) -> Dict:
        """
        Transform Zerodha position data to MarvelQuant format.
