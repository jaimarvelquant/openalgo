...
class BrokerData:
    ...
    def get_quotes(self, symbol_list, timeout: int = 5) -> List[Dict[str, Any]]:
        ...
        # Special case for MarvelQuant standard format: direct quote request via Bruno
                ...
                if token:
                    ...
                    if websocket and websocket.is_connected:
                        ...
                        if success:
                            ...
                            if quote:
                                # Format the response according to MarvelQuant standard format
                                quote_item = {
                                    'symbol': symbol,
                                    'exchange': exchange,
                                    'token': token,
    ...
    def get_depth(self, symbol_list, timeout: int = 5):
        ...
        Returns:
            Dict with market depth data in the MarvelQuant standard format
        """
            ...
            if isinstance(sym, dict) and 'symbol' in sym and 'exchange' in sym:
                ...
                if token:
                    ...
                    if success:
                        ...
                        if depth:
                            # Create a normalized depth structure in the MarvelQuant format
                            item = {
                                'symbol': symbol,
                                'exchange': exchange,
                                'token': token,
