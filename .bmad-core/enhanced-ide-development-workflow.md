...
5. **Complete implementation** and run tests
6. **Update story status** to "Review"
7. **Execute**: `*qa-handoff` (prepares standardized handoff)

## QA Review Process (Test Architect)

After developer completes implementation and executes `*qa-handoff`:

### QA Review Trigger
1. **Receive dev handoff** with completion summary
2. **Validate handoff completeness** (tests passing, file list updated, etc.)
3. **Execute**: `*review {story-id}` (comprehensive quality assessment)

### QA Review Outputs
- **QA Results section** added to story file
- **Quality gate file** created in `docs/qa/gates/`
- **Assessment reports** in `docs/qa/assessments/` (optional)
- **Story status recommendation** (Done or changes needed)

### Post-QA Actions
- If **PASS**: Story status → "Done"
- If **CONCERNS/FAIL**: Dev addresses feedback, then re-request review
- If **WAIVED**: Document business justification for proceeding

## Iterative QA Review Cycle

When QA finds issues requiring fixes, follow this structured feedback loop:

### QA Finds Issues (CONCERNS/FAIL)

1. **QA updates story** with findings in QA Results section
2. **QA creates/updates gate file** with new status and issues
3. **QA provides clear feedback** on what needs to be fixed

### Dev Implements Fixes

1. **Dev analyzes QA feedback** from story and gate file
2. **Dev executes**: `*qa-feedback` (follows qa-feedback-loop.md)
3. **Dev implements fixes** following priority order:
   - CRITICAL: Security, functionality, data issues
   - HIGH: AC violations, API changes
   - MEDIUM: Test coverage, performance
   - LOW: Style, documentation
4. **Dev validates fixes** with comprehensive testing
5. **Dev updates story** with fix details and file changes
6. **Dev requests re-review**: `*qa-handoff` then `@qa *review {story}`

### QA Re-Reviews

1. **QA validates fixes** against original findings
2. **QA checks code quality** and regression testing
3. **QA updates assessment**:
   - **PASS**: All issues resolved
   - **CONCERNS**: Minor issues remain (acceptable)
   - **FAIL**: Critical issues still present
   - **WAIVED**: Issues accepted with justification
4. **QA updates gate file** and story status

### Cycle Limits

- **Soft Limit**: 3 review cycles per story
- **Hard Limit**: 5 review cycles per story
...
- **Decision:** CONCERNS
- **Issues:** 3 findings identified

### Round 2: {timestamp} - After Fixes
- **Decision:** PASS
- **Issues Resolved:** All 3 findings addressed
```
...
# Round 1
gate: CONCERNS
status_reason: 'Test coverage below target'
top_issues:
  - id: 'TEST-001'
    severity: medium
    finding: 'Integration test coverage 75% (target 80%)'

# Round 2
gate: PASS
status_reason: 'All QA feedback addressed, coverage now 82%'
top_issues: []  # Cleared after fixes
```
...
## Epic Validation and Retrospective

After all stories in an epic are marked "Done":

### Epic Completion Validation
1. **PO**: Validate epic completion (`*validate-epic-completion {epic-id}`)
2. **PO**: Review all story statuses, QA gates, and acceptance criteria
3. **PO**: Assess epic goals achievement and quality metrics
4. **PO**: Generate epic completion report and deployment readiness

### Epic Retrospective
1. **PO**: Conduct structured retrospective (`*epic-retrospective {epic-id}`)
2. **PO**: Analyze what went well and what could be improved
3. **PO**: Identify lessons learned and process improvements
4. **PO**: Create action items for future epics

### Final Approval
1. **PO**: Approve epic for production deployment
2. **PO**: Document any follow-up work or remediation needed
3. **Team**: Plan next epic or celebrate project completion
