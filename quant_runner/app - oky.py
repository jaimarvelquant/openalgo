...
def duplicate_strategy(strategy_id):
    with db_connect() as conn:
        ...
        """, (
            strat_dict["instance_name"] + "_copy",
            strat_dict["symbol"],
        ...
        # Duplicate legs
        cursor.execute("SELECT leg_id, leg_str FROM strategy_legs WHERE strategy_id=?", (strategy_id,))
        legs = cursor.fetchall()
        for leg_id, leg_str in legs:
            cursor.execute("INSERT INTO strategy_legs (strategy_id, leg_id, leg_str) VALUES (?, ?, ?)",
                           (new_id, leg_id, leg_str))
        conn.commit()
...
def place_slm_order(strategy, leg, entry_resp, avg_price, leg_id):
    try:
        ...
        with db_connect() as conn:
            ...
            cursor.execute("""
                UPDATE leg_status
                SET sl_order_id=?, stoploss_price=?, sl_hit=0, status=?, entry_time=?
        ...
        with db_connect() as conn:
            ...
            cursor.execute("""
                UPDATE leg_status
                SET sl_order_id=NULL, stoploss_price=?, sl_hit=0, status='ERROR'
                WHERE strategy_id=? AND leg_id=?
            """, (
                avg_price,
                strategy["id"],
...
def place_leg_workflow(strategy, leg_str, leg_id):
    try:
        ...
        if leg_qty != leg["quantity"]:
            print(f"[WARN] Adjusted quantity for leg {leg_id} from {leg['quantity']} to {leg_qty}")
        leg["quantity"] = leg_qty

        # Get freeze qty and normalize splitsize
        ...
        order_status = resp.get("status", "UNKNOWN")
        ...
        avg_price = 0
        for _ in range(10):
            avg_price = fetch_avg_price(order_id, strategy["instance_name"])
            if avg_price > 0:
                break
            time.sleep(3)
        ...
        if avg_price > 0:
            ...
            # Fallback: use entry price if avg_price never comes
